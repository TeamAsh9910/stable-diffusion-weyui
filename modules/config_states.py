"""
Supports saving and restoring weyui and extensions from a known working set of commits
"""
...
def get_weyui_config():
    weyui_repo = None

    try:
        if os.path.exists(os.path.join(script_path, ".git")):
            weyui_repo = git.Repo(script_path)
    except Exception:
        errors.report(f"Error reading weyui git info from {script_path}", exc_info=True)

    weyui_remote = None
    weyui_commit_hash = None
    weyui_commit_date = None
    weyui_branch = None
    if weyui_repo and not weyui_repo.bare:
        try:
            weyui_remote = next(weyui_repo.remote().urls, None)
            head = weyui_repo.head.commit
            weyui_commit_date = weyui_repo.head.commit.committed_date
            weyui_commit_hash = head.hexsha
            weyui_branch = weyui_repo.active_branch.name
    ...
    return {
        "remote": weyui_remote,
        "commit_hash": weyui_commit_hash,
        "commit_date": weyui_commit_date,
        "branch": weyui_branch,
    }
...
def restore_weyui_config(config):
    print("* Restoring weyui state...")

    if "weyui" not in config:
        print("Error: No weyui data saved to config")
        return

    weyui_config = config["weyui"]

    if "commit_hash" not in weyui_config:
        print("Error: No commit saved to weyui config")
        return

    weyui_commit_hash = weyui_config.get("commit_hash", None)
    weyui_repo = None
    ...
    try:
        weyui_repo.git.fetch(all=True)
        weyui_repo.git.reset(weyui_commit_hash, hard=True)
        print(f"* Restored weyui to commit {weyui_commit_hash}.")
    except Exception:
        errors.report(f"Error restoring weyui to commit{weyui_commit_hash}")
