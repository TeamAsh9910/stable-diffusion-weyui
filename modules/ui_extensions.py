...
def save_config_state(name):
    ...
    print(f"Saving backup of weyui/extension state to {filename}.")
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(current_config_state, f, indent=4)
    config_states.list_config_states()
    new_value = next(iter(config_states.all_config_states.keys()), "Current")
    new_choices = ["Current"] + list(config_states.all_config_states.keys())
    return gr.Dropdown.update(value=new_value, choices=new_choices), f"<span>Saved current weyui/extension state to \"{filename}\"</span>"
...
def restore_config_state(confirmed, config_state_name, restore_type):
    ...
    print(f"*** Restoring weyui state from backup: {restore_type} ***")

    if restore_type == "extensions" or restore_type == "both":
        shared.opts.restore_config_state_file = config_state["filepath"]
        shared.opts.save(shared.config_filename)

    if restore_type == "weyui" or restore_type == "both":
        config_states.restore_weyui_config(config_state)
...
def update_config_states_table(state_name):
    ...
    try:
        weyui_remote = config_state["weyui"]["remote"] or ""
        weyui_branch = config_state["weyui"]["branch"]
        weyui_commit_hash = config_state["weyui"]["commit_hash"] or "<unknown>"
        weyui_commit_date = config_state["weyui"]["commit_date"]
        if weyui_commit_date:
            weyui_commit_date = time.asctime(time.gmtime(weyui_commit_date))
        else:
            weyui_commit_date = "<unknown>"

        remote = f"""<a href="{html.escape(weyui_remote)}" target="_blank">{html.escape(weyui_remote or '')}</a>"""
        commit_link = make_commit_link(weyui_commit_hash, weyui_remote)
        date_link = make_commit_link(weyui_commit_hash, weyui_remote, weyui_commit_date)

        current_weyui = config_states.get_weyui_config()

        style_remote = ""
        style_branch = ""
        style_commit = ""
        if current_weyui["remote"] != weyui_remote:
            style_remote = STYLE_PRIMARY
        if current_weyui["branch"] != weyui_branch:
            style_branch = STYLE_PRIMARY
        if current_weyui["commit_hash"] != weyui_commit_hash:
...
<table id="config_state_weyui">
    ...
    <tbody>
        <tr>
            ...
            <td>
                <label{style_branch}>{weyui_branch}</label>
            </td>
            <td>
        ...
        with gr.Tabs(elem_id="tabs_extensions"):
            ...
            with gr.TabItem("Available", id="available"):
                with gr.Row():
                    ...
                    extensions_index_url = os.environ.get('WEBUI_EXTENSIONS_INDEX', "https://raw.githubusercontent.com/AUTOMATIC1111/stable-diffusion-weyui-extensions/master/index.json")
            ...
            with gr.TabItem("Backup/Restore"):
                with gr.Row(elem_id="extensions_backup_top_row"):
                    ...
                    config_restore_type = gr.Radio(label="State to restore", choices=["extensions", "weyui", "both"], value="extensions", elem_id="extension_backup_restore_type")
